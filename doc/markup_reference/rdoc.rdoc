...
- {Annotation}[rdoc-ref:rdoc@Annotations]: information about an abstract object or an overridden method.
...
Annotations describe relationships that RDoc cannot determine from source code
alone. Put an annotation on its own line at the comment margin in a class,
module, or method comment. RDoc removes the annotation line from the displayed
comment.

Use <tt>@abstract</tt> on a class, module, or method:

  # A component that subclasses must implement.
  #
  # @abstract
  class Component
...
Use <tt>@override</tt> on a method that replaces an ancestor method:
...
RDoc resolves instance methods through included modules and superclasses in
Ruby's method lookup order. For singleton methods, RDoc checks extended modules
and superclass singleton methods. If the ancestor method is documented, Aliki
and +ri+ show a link or the full method name. Aliki also lists known subclasses
and known overrides while it generates a complete documentation set.

The annotations are supported in Ruby, C, and RBS source comments. They do not
take arguments. A line such as <tt>@abstract because subclasses differ</tt>
remains part of the displayed comment.
